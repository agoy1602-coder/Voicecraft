...
const replacement = String.raw`// [VoiceCraft] offline persistence downloader v7
const __VC_POCKET_V7_DB = 'voicecraft-pocket-tts-v7';
const __VC_POCKET_V7_STORE = 'assets';

function __vcPocketV7OpenDb() {
    return new Promise((resolve, reject) => {
        try {
            const request = indexedDB.open(__VC_POCKET_V7_DB, 1);
            request.onupgradeneeded = () => {
                if (!request.result.objectStoreNames.contains(__VC_POCKET_V7_STORE)) request.result.createObjectStore(__VC_POCKET_V7_STORE);
            };
...
async function __vcPocketV7Get(url) {
    try {
        const db = await __vcPocketV7OpenDb();
        return await new Promise((resolve, reject) => {
            const tx = db.transaction(__VC_POCKET_V7_STORE, 'readonly');
            const req = tx.objectStore(__VC_POCKET_V7_STORE).get(url);
...
async function __vcPocketV7Put(url, bytes) {
    try {
        const db = await __vcPocketV7OpenDb();
        await new Promise((resolve, reject) => {
            const tx = db.transaction(__VC_POCKET_V7_STORE, 'readwrite');
            tx.objectStore(__VC_POCKET_V7_STORE).put(bytes.buffer, url);
...
async function fetchWithProgress(url, label, onProgress) {
    ...
    const idbHit = await __vcPocketV7Get(url);
    if (idbHit) {
    ...
    const persisted = await __vcPocketV7Put(url, bytes);
...
console.log('[VoiceCraft] installed Pocket TTS offline persistence downloader v7');
