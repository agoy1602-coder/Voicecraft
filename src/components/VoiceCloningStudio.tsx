              ...
              <div className="flex flex-col items-center justify-center gap-4 py-4">
                ...
                <div className="bg-slate-900/60 p-3 rounded-lg border border-slate-800 text-center w-full">
                  ...
                  <p className="text-xs text-slate-200 font-medium italic">
                    "The quick brown fox jumps over the lazy dog and explores the northern aurora sky."
                  </p>
                </div>
              ...
              <div className="flex flex-col items-center justify-center gap-3 py-6 border-2 border-dashed border-slate-800 rounded-xl hover:border-violet-500/50 transition-colors p-4">
                ...
                <div className="text-center">
                  <p className="text-xs font-semibold text-slate-200">
                    Upload an audio sample (.wav, .mp3, .m4a, .ogg)
                  </p>
                ...
                <input
                  ...
                  className="text-xs text-slate-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-violet-600 file:text-white hover:file:bg-violet-500 cursor-pointer"
                />
                {recordedBlob && (
                  <div className="flex items-center gap-1.5 text-xs text-emerald-400 mt-2 font-medium">
                    <CheckCircle2 className="w-4 h-4" /> File loaded: {(recordedBlob.size / 1024).toFixed(0)} KB
                  </div>
                )}
              </div>
            )}
          ...
          <div className="flex flex-col gap-4 bg-slate-950/80 p-5 rounded-xl border border-slate-800 justify-between">
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  id="cloned-voice-name-input"
                  type="text"
                  value={voiceName}
                  onChange={(e) => setVoiceName(e.target.value)}
                  placeholder="e.g. Marcus (Deep Narrator) or Sarah (Studio)"
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500 transition-all font-medium"
                />
              </div>

              {/* Personality Notes */}
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-slate-300">Acoustic Tone & Cadence Notes</label>
                <textarea
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="e.g. Warm baritone with slow, thoughtful pauses and slight vocal fry."
                  rows={2}
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3.5 py-2 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500/50 focus:border-violet-500 transition-all resize-none font-medium"
                />
              </div>

              <div className="flex items-center gap-2 p-2.5 rounded-lg bg-violet-950/30 border border-violet-500/20 text-[11px] text-violet-300">
                <ShieldCheck className="w-4 h-4 text-violet-400 shrink-0" />
                <span>All acoustic fingerprints are stored encrypted via client AES-256-GCM.</span>
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full py-3 rounded-xl font-bold text-xs text-white flex items-center justify-center gap-2 shadow-lg transition-all ${
                isAnalyzing || !voiceName.trim() || (!recordedBlob && !recordedBase64)
                  ? 'bg-slate-800 text-slate-500 cursor-not-allowed'
                  : 'bg-gradient-to-r from-amber-500 to-rose-600 hover:from-amber-400 hover:to-rose-500 shadow-amber-500/20 active:scale-98'
              }`}
            >
              {isAnalyzing ? (
                <>
                  <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  <span>{analysisStatus || 'Analyzing Acoustic Blueprint...'}</span>
                </>
              ) : (
                <>
                  <Sparkles className="w-4 h-4" />
                  <span>Create Personalized Voice Clone</span>
                </>
              )}
            </button>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-base font-bold text-slate-100 flex items-center gap-2">
              ...
              Your Custom Cloned Voices ({clonedVoices.length})
            </h3>
            <p className="text-xs text-slate-400">
              Personalized voices ready to apply to any text-to-speech script
            </p>
          </div>
        </div>

        {clonedVoices.length === 0 ? (
          <div className="bg-slate-950/60 rounded-xl p-8 text-center border border-slate-800/80 flex flex-col items-center justify-center">
            <Info className="w-8 h-8 text-slate-600 mb-2" />
            <p className="text-sm font-semibold text-slate-300">No Cloned Voices Created Yet</p>
            <p className="text-xs text-slate-500 max-w-sm mt-1">
              Record a 5-second sample above to create your first personal acoustic clone profile.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {clonedVoices.map((voice) => (
              <div
                key={voice.id}
                className="bg-slate-950/80 border border-slate-800 hover:border-slate-700 rounded-xl p-4 flex flex-col justify-between gap-3 transition-all group"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-xl bg-gradient-to-tr ${voice.avatarColor} flex items-center justify-center font-bold text-white text-xs shadow-md shrink-0`}
                    >
                      {voice.name.substring(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <h4 className="text-xs font-bold text-slate-100">{voice.name}</h4>
                      <p className="text-[10px] text-amber-300 font-medium capitalize">
                        {voice.tone} tone • {voice.gender}
                      </p>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg text-slate-500 hover:text-rose-400 hover:bg-slate-800/80 transition-colors opacity-80 group-hover:opacity-100"
                  >
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-800/60 text-[10px] text-slate-400">
                  <span>Base F0: {voice.basePitchHz || 160} Hz</span>
                  <button
                    onClick={() => onSelectForTTS(voice)}
                    className="px-3 py-1 rounded-lg bg-violet-600/80 hover:bg-violet-500 text-white font-semibold flex items-center gap-1 transition-all"
                  >
                    <Play className="w-3 h-3 ml-0.5" /> Use in TTS
                  </button>
                </div>
              </div>
            ))}
