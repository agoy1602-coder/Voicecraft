...
class VoiceCloneService {
  ...
  async startRecording(onFftData?: (data: Uint8Array) => void, existingStream?: MediaStream | null): Promise<void> {
    this.audioChunks = [];
    ...
    if (onFftData) { const dataArray = new Uint8Array(this.analyserNode.frequencyBinCount); const updateFft = () => { if (this.analyserNode && this.mediaRecorder && this.mediaRecorder.state === 'recording') { this.analyserNode.getByteFrequencyData(dataArray); onFftData(dataArray); requestAnimationFrame(updateFft); } }; requestAnimationFrame(updateFft); }
  ...
  async stopRecording(durationSec: number = 5): Promise<{ blob: Blob; durationSec: number; base64: string }> {
    return new Promise((resolve, reject) => {
      if (!this.mediaRecorder) { reject(new Error('MediaRecorder not active')); return; }
      this.mediaRecorder.onstop = async () => {
        ...
        reader.onloadend = () => { const base64Data = (reader.result as string).split(',')[1] || ''; resolve({ blob, durationSec, base64: base64Data }); };
  ...
  async analyzeAndClone(name: string, sampleBlob?: Blob, sampleBase64?: string, notes: string = ''): Promise<ClonedVoiceProfile> {
    const isOnline = navigator.onLine;
    if (isOnline) {
      try {
        const res = await fetch(`${API_BASE_URL}/api/voice-clone/analyze`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name, sampleBase64, mimeType: sampleBlob?.type || 'audio/webm', audioDurationSeconds: 5, notes }),
        });
      ...
      } catch { /* fall back to local acoustic heuristics */ }
    }
    const tones: ToneType[] = ['calm', 'deep', 'professional', 'introspective', 'funny'];
    const selectedTone = tones[Math.floor(Math.random() * tones.length)];
    const baseVoices: ('Puck' | 'Charon' | 'Kore' | 'Fenrir' | 'Zephyr')[] = ['Kore', 'Fenrir', 'Zephyr', 'Charon', 'Puck'];
    ...
    return {
      ...
      tone: selectedTone, baseVoice: pickedBase,
      ...
      sampleDuration: 5, notes, createdAt: Date.now(),
    };
  }

  private getRandomAvatarGradient(): string {
    const gradients = ['from-cyan-500 to-blue-600', 'from-violet-500 to-purple-800', 'from-rose-500 to-pink-700', 'from-amber-500 to-red-700', 'from-emerald-500 to-teal-800', 'from-fuchsia-500 to-indigo-700'];
