...
class VoiceCloneService {
  ...
  /**
   * Create the durable clone profile without initializing the ~150 MB Pocket TTS
   * inference engine. The reference recording is the source of truth; Pocket TTS
   * is initialized lazily on the first synthesis. This keeps Create Clone usable
   * offline and prevents model loading from freezing the UI.
   */
  async analyzeAndClone(
    ...
    const profile: ClonedVoiceProfile = {
      ...
      tone: 'professional',
      baseVoice: 'Zephyr',
      description: `Private browser-local neural voice clone for ${name.trim()}.`,
      ...
      promptModifier: `Speak naturally using the cloned voice identity for ${name.trim()}.`,
      sampleDuration: 5,
      notes,
      createdAt: Date.now(),
      provider: 'pocket-tts',
      // Runtime voice IDs are session-scoped and are intentionally created lazily.
